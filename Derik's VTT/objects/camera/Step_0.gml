...
//show_debug_message(global.selection_action);
//Clear Inputs
...
var stw_cast = undefined;
if !ImGui.IsAnyItemHovered() and !ImGui.IsWindowHovered(ImGuiHoveredFlags.AnyWindow) {
	
	//Screen to World Cast
	stw_cast = mouse_to_world_cast();
	
	//Store Ray Hit Position in Vec3
	stw_cast_pos = new vec3(stw_cast[1], stw_cast[2], stw_cast[3]);
	stw_cast_norm = new vec3(stw_cast[4], stw_cast[5], stw_cast[6]);
	stw_cast_success = stw_cast[0];
...
else  //Everything else
{
	...
	//Asset Quick Functions
	var inst =  global.selected_inst;
	if instance_exists(inst) {
	
	//Store Asset Type
	var inst_asset_type =  inst.type;
	
	//Button Settings
	var button_height = ImGui.GetWindowHeight()+2;
	...
	ImGui.PushStyleVar(ImGuiStyleVar.ItemSpacing, button_width/3, button_width/2);
	ImGui.PushStyleVar(ImGuiStyleVar.CellPadding, 0, 0);
	var menubar_color = ImGui.GetStyleColor(ImGuiCol.MenuBarBg);
	var hovered_color = ImGui.GetStyleColor(ImGuiCol.HeaderHovered);
	var mouse_released = mouse_check_button_released(mb_left);
	var hovered_scale = 0.75;

		//Players & NPCs
		if inst_asset_type = asset_types.player or inst_asset_type = asset_types.npc {

			//Teleport 
			var teleport_c = c_white; if teleport_hovered {teleport_c = hovered_color;};
			if ImGui.ImageButton("teleport", action_icons, 0, teleport_c, 1, menubar_color, 1, button_width, button_height) {
				global.selection_action = asset_actions.teleport;
			}
			teleport_hovered = ImGui.IsItemHovered();
			...
			look_dir_hovered = ImGui.IsItemHovered();
	...
	//Walking Distance Window
	if global.selection_action = asset_actions.walk {walking_distance_open = true;} else {walking_distance_open = false;}
	if walking_distance_open {
	var mid_point = walk_start_pos.Lerp(walk_target_pos, 0.5);
	var mid_point_screen = world_to_screen(mid_point, mat_view, mat_projection);
	var window_pos = mid_point_screen.Sub(walking_dist_offset);
	var walking_dist = string(walk_steps * 2.5) + " ft.";	
	ImGui.SetNextWindowPos(window_pos.x, window_pos.y);
	ImGui.Begin("Walking Distance", true, ImGuiWindowFlags.Modal | ImGuiWindowFlags.NoNav | ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoMove);
	ImGui.Text(walking_dist);
	walking_dist_offset = new vec2(ImGui.GetWindowWidth(), ImGui.GetWindowHeight()).Mul(0.5);
	ImGui.End();
	}
